...
5. Если нужно спарсить только города для регионов (не тарифы), раскомментируйте строку с `new RootStage(citiesOnlyConfig).go()` и закомментируйте (необязательно) запуск других `RootStage`.
...
9. Если после ошибки на одной из страниц парсер не восстановил работу сам или зациклился на одной и той же ошибке, посмотрите в соответсвующих логах номер региона и номер последнего города. Введите в консоль команду формата `[restart|stop|start] [main|internet|rancho] [номер региона (необязательно)] [номер города (необязательно)]`

   9.1 При зацикливании `restart`, при зависании закройте или обновите страницу и затем `restart`.

   9.2 Чтобы не поретять логи, сначала `stop`, скопируйте логи куда-нибудь, затем `start`.
